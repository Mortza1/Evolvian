...
@app.post("/api/operations", response_model=OperationResponse, status_code=status.HTTP_201_CREATED)
async def create_operation(
    ...
    operation = Operation(
        ...
        started_at=datetime.utcnow() if operation_data.status == "active" else None
    )
...
@app.patch("/api/operations/{operation_id}", response_model=OperationResponse)
...
@app.delete("/api/operations/{operation_id}", status_code=status.HTTP_204_NO_CONTENT)
